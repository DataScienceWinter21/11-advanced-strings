...
tw_noMentions <- tweets %>%
  mutate(textNoMentions = str_replace_all(text, Mentions, ""))
...
*answer:* The regular expression `reg` matches anything but `A-Za-z\\d#@'` or `'` that do not have `A-Za-z\\d#@` after.
...
```{r}
tweet_words <- tweets %>%
  filter(!str_detect(text, '^"')) %>%   # not starting with " or ^
  mutate(text = str_replace_all(text, Links, "")) %>%
  unnest_tokens(word, text, token = "regex", pattern = reg) %>%
  filter(!word %in% stop_words$word, str_detect(word, "[a-z]"))  # don't want any numbers
  
```

### c. Find the proportion of the word "America" in this final tibble. Why does this proportion not match with the one you got in 3a?


```{r}
tweet_words %>% 
  summarize(prop = mean(str_detect(str_to_title(word),"America"))) 
```

*answer:* The answers do not match because we got rid of stop words.
